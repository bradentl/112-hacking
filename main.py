...
def timerFired(app):
	# Useful for activating code every x time constants
	app.timer = (app.timer + 1) % 10

	if(not app.board):
		return

	if(app.board.player.target and (app.timer % 2) == 0):
		# Will constant calls significantly impact efficiency?
